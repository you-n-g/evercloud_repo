...
CloudApp.controller('InstancemanageController',
	...
	$scope.instancemanages = [];
        ...
        $scope.openBatchNewModal = function () { 
           $modal.open({
                templateUrl: '/static/management/views/instance_batch_wizard/instance_wizard.html?t=' + Math.random(),
	...
	$scope.socket_list = [1,2,4,8];
        ...
        $scope.instance_config.GPU = false;
        ...
        $scope.calculateCost = function(config){
            ...
            $scope.submit_click = function(instance_config){
                var params_data = {"id":$scope.instancemanage.id,
					...
					"core":instance_config.core,
					"socket":instance_config.socket,
                                        "sys_disk": $scope.instancemanage.sys_disk,
					"memory":instance_config.memory};
        ...
        function(ValidationTool, $i18next){
            return {
                init: function(){
                    ...
                    return ValidationTool.init('#submit_form', config);
                }
            }
        }])
    .controller('InstancemanageAssignController',
    ...
    function ($rootScope, $scope, $state, $filter, $interval, lodash,
              ...
              CommonHttpService, PriceTool, Instance, Image, Flavor, Network, instancemanage_table,
              //instance_table,
              CheckboxGroup,ngTableHelper, User, MemberUser,
	      quota, cpuPrices, memoryPrices) {
        ...
        $scope.user_table = new ngTableParams({
                ...
                getData: function ($defer, params) {
                    ...
                    MemberUser.query(searchParams, function (data) {
	...
	$scope.socket_list = [1,2,4,8];
        ...
        var user_ids =  function(){
                ...
                $scope.instance_config.user_ids = ids;
                $scope.user_count = user_count;
            }
        $scope.submit_click = function (instance_config) {
            user_ids();
            if ($scope.user_count == 0) {
                  ToastrService.warning("Please select at least 1 user");
                  return 0;
            };
            var post_data = {
                "name": instance_config.name,
                ...
                "pay_type": instance_config.pay_type,
                "pay_num": instance_config.pay_num,
                "user_ids": $scope.instance_config.user_ids
            };
            CommonHttpService.post("/api/instances/batch_create/", post_data).then(function (data) {
                if (data.OPERATION_STATUS == 1) {
                    ToastrService.success(data.msg, $i18next("success"));
                    $modalInstance.dismiss();
                    instancemanage_table.reload();
        ...
        $scope.calcuate_resource_persent = function (resource) {
            if (quota[resource] <= 0) {
                ...
                var current = $scope.instance_config[resource];
                user_ids();
                $scope.instance_config.instance = $scope.user_count;
                if (resource != "instance")
                    //current = current * $scope.instance_config.instance;
                    current = current * $scope.user_count;
                return (quota[resource + "_used"] + current) / quota[resource] * 100;
