...
class UserProxy(User):
    ...
    @property
    def has_udc(self):
        return self.has_any_data_center(self)
    ...
    def has_any_data_center(cls, user):
        return UserDataCenter.objects.filter(user=user).exists()
...
class Contract(LivingDeadModel):
    ...
    def __unicode__(self):
        return u"<Contract ID:%s Name:%s>" % (self.id, self.name)
...
class Quota(LivingDeadModel):
    ...
    def __unicode__(self):
        return u"<Quota ID:%s Contract:%s %s=%s>" % (self.id,
                    self.contract.name, self.resource, self.limit)
...
class Operation(models.Model):
    ...
    @property
    def resource_i18n(self):
        return _(self.resource)

    @property
    def action_i18n(self):
        return _(self.action)
